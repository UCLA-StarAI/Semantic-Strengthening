...
def recover(constraints, input, model, K=5, var_map=None, timings=None, caution=False, limit=100000, threshold=0.01):
    ...
    for i, (c1, c2) in enumerate(itertools.combinations(constraints, 2)):
        ...
        #if timings != None and (timings[c1] > 0.1 or timings[c2] > 0.1):
        #if timings != None and (timings[c1] > 0.01 or timings[c2] > 0.01):
        if timings != None and (timings[c1] > threshold or timings[c2] > threshold):
            continue

        # If the constraints are defined over
        # disjoint sets of variables, they are
        # independent
        if caution and var_map[c1].isdisjoint(var_map[c2]):
            continue
