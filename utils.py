...
class LazyTrainingIterator(object):
    def __init__(self, basefile_dict):
        self.array_loader_dict = {k: BigArrayLoader(basefile) for k,basefile  in basefile_dict.items()}
    def get_epoch_iterator(self, batch_size, number_of_epochs, shuffle=True, device='cpu', preload=False):
        def iterator():
            for e in range(number_of_epochs):
                for i, zipped_data in enumerate(zip(*self.array_loader_dict.values())):
                    data_chunk_dict = dict(zip(self.array_loader_dict.keys(),zipped_data))
                    yield from batch_iterator(data_chunk_dict, batch_size, shuffle, device=device, preload=preload)
        return iterator()


class TransformTrainingIterator(TrainingIterator):
    def __init__(self, data_dict, transform_func_dict, **kwargs):
        super().__init__(data_dict, **kwargs)
        self.func_dict = func_dict
    def get_epoch_iterator(self, batch_size, number_of_epochs, shuffle=True, **kwargs):
        epoch_iterator = super().get_epoch_iterator(batch_size, number_of_epochs, shuffle, **kwargs)
        def iterator():
            for batch in epoch_iterator:
                # apply transformation
                for k, trans in self.func_dict.items():
                    batch[k] = trans(batch[k])
                yield batch
        return iterator()

class TransformLazyTrainingIterator(LazyTrainingIterator):
    def __init__(self, basefile_dict, transform_func_dict, **kwargs):
        super().__init__(basefile_dict, **kwargs)
        self.func_dict = transform_func_dict
    def get_epoch_iterator(self, batch_size, number_of_epochs, shuffle=True, **kwargs):
        epoch_iterator = super().get_epoch_iterator(batch_size, number_of_epochs, shuffle, **kwargs)
        def iterator():
            for batch in epoch_iterator:
...
def all_accuracies(true_labels, suggested_labels, true_costs, is_valid_label_fn, num_thresholds, minimize=True):
    ...
    for true_label, suggested_label, true_cost in zip(true_labels, suggested_labels, true_costs):
        ...
        if cost_ratio < 0.99:
            print("cost_ratio < 0.99...", cost_ratio)
            if cost_ratio < 1.0:
    ...
    threshold_dict['indiv_acc'] = indiv / (num_examples * 264)
